...
Три основных экрана. Значения вымышленные: адреса взяты из диапазонов, зарезервированных под документацию (RFC 5737), имена прокси и версии — для примера.

**Главный экран.** Открывается по `/menu` или кнопке `🏠 Меню`. Сразу видно, что за роутер, какой форк podkop, какой прокси активен и каким путём сам бот ходит в Telegram.

```text
🖥️ Управление Podkop
Устройство: AX6000
Podkop: 0.7.14-r1 (Forkop) | Бот: v0.19.9
Секция: main
