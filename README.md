...
[![version](https://img.shields.io/badge/version-0.19.11-blue?style=flat-square)](CHANGELOG_RUS.md)
...
Три основных экрана. Примеры сняты с **оригинального podkop** (itdoginfo), поэтому версия здесь в схеме `0.7.x` — у форков она своя: у Forkop, например, `1.0.x`. Строки после `клавиатура:` — это кнопки под сообщением; знак `·` разделяет две кнопки, стоящие в одном ряду. Адреса взяты из диапазонов, зарезервированных под документацию (RFC 5737), имена прокси вымышленные.

**Главный экран.** Открывается по `/menu` или кнопке `🏠 Меню`. Сразу видно, что за роутер, какой вариант podkop, какой прокси активен и каким путём сам бот ходит в Telegram.

```text
🖥️ Управление Podkop
Устройство: AX6000
Podkop: 0.7.14-r1 (Podkop) | Бот: v0.19.11
Секция: main
...
▸ 🤖 Настройки бота · 🛑 Остановить
▸ 🔧 Обслуживание
```

> В скобках после версии бот показывает, какой вариант он определил: `Podkop`, `Podkop Evolution`, `NetShift`, `Podkop Plus` или `Forkop`. На Plus и Forkop в клавиатуре добавляется ещё ряд `🖧 Службы`.

**📊 Статус.** Первая строка — общий вердикт, дальше подробности. Здесь показан характерный случай: провайдер режет Telegram напрямую, но бот жив, потому что ходит через туннель podkop.
...
клавиатура:
▸ 🧭 Подробнее
▸ ♻️ Перезапустить Podkop
▸ 🔃 Обновить · 🏠 Меню
```

**🌐 Прокси.** Активный помечен `▶` и выделен жирным, у остальных — цветная точка по задержке: 🟢 до 200 мс, 🟡 до 500, 🟠 до 900, 🔴 выше. Кнопки повторяют разметку списка, поэтому не нужно сверять номер со строкой перед нажатием.
...
> Полный справочник всех экранов с кнопками и полями — в [BOT_STRUCTURE.md](BOT_STRUCTURE.md).

---

## 🗺️ Главное меню

Кнопки главного экрана стоят в два столбца — здесь они в одной строке через `·`:

```text
🖥️ Управление Podkop
├─ 📊 Статус · 🌐 Прокси
├─ ⚙ Настройки · 🔄 Перезапустить
├─ 🤖 Настройки бота · 🛑 Остановить (или 🟢 Запустить)
...
🌐 Прокси
├─ заголовок карточки зависит от режима секции: «Прокси · Selector»,
│  «Прокси · URLTest», «Прокси подписки» или «Прокси с одной ссылкой»
...
│  URLTest, DNS-резолвер, включение секции и автозапуска. На Forkop
│  добавляются 🎯 Условия и 🔗 Через секцию, на Plus — 🔬 Фильтры URLTest
...
🧪 Диагностика — путь: 📊 Статус → 🧭 Подробнее → 🧪 Диагностика
├─ 🩺 Состояние туннеля
